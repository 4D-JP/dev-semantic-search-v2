...
If (False:C215)
	...
	For each ($line; $jsonl)
		...
		var $passage : 4D:C1709.Entity
		$passage:=ds:C1482.Passage.get(Num:C11($id))
		If ($passage=Null:C1517)
			continue
		End if 
		
		var $trainingPassage : cs:C1710.TrainingPassageEntity
		$trainingPassage:=ds:C1482.TrainingPassage.new()
		$trainingPassage.document:=$passage.document
		$trainingPassage.embeddings:=$passage.embeddings
		$trainingPassage.text:=$passage.text
		$trainingPassage.hash:=$passage.hash
		$trainingPassage.meta:=$passage.meta
		$trainingPassage.save()
		...
		For each ($result; $results)
			...
			If ($batch.success)
				...
				$search.meta:={model: "bge-m3"; provider: "llama.cpp"}
				$search.save()
