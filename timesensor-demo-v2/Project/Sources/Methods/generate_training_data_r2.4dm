...
*/
			$negativePassages:=ds:C1482.Passage.query("embeddings > :1"+\
				"    and meta.provider  == :2"+\
				"    and meta.model == :3"+\
				"    and not(DocumentID in :4)"+\
				"    and not(hash in :5)"+\
				"    and not(hash in :6)"; $comparison; $provider; $model; \
				$documentIds; $positiveHashes; $negativeHashes)
