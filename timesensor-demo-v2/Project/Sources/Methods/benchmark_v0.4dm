...
$s:=ds:C1482.Search.query("vectors.meta.provider == :1"; "OpenAI")
...
For each ($search; $searches)
	...
	$embeddings:=$search.vectors.query("meta.provider == :1"; "OpenAI").first().embeddings
	...
	$documents:=ds:C1482.Full.query("passages.meta.provider == :1 "+\
		...
		" and passages.embeddings > :3"; "OpenAI"; $search.language; $comparison; $queryParams)
	$r:=$documents.passages.and($search.passage)
	...
	If ($r.length#0)
		...
		$rank:=$search.passage.indexOf($oo)
		If ($rank#-1)
			$stat.rank:=$rank+1
