...
$hashes:=ds:C1482.Search.query("meta.provider == :1"; "OpenAI").distinct("hash")
//
While ($count*$batch<$hashes.length)
	...
	For each ($hash; $queryHashes)
		...
		$searches:=ds:C1482.Search.query("hash == :1 and positive == :1"; $hash; True:C214)
		...
		For each ($search; $searches)
			...
			$hardNegatives:=ds:C1482.Search.query("embeddings > :1"+\
