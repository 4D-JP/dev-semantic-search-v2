...
#  Usage: bash setup_and_run.sh r3
...
# ── Model + dataset ───────────────────────────────────────────────────────────
# BASE_MODEL: the model to fine-tune from.
#   Fresh run from base:        BASE_MODEL="BAAI/bge-m3"
#   Continuing from prior round: BASE_MODEL="${HF_USER}/bge-m3-lemur-r1-merged"
BASE_MODEL="BAAI/bge-m3"

# HF_DATASET: the training dataset on HuggingFace.
#   Defaults to the dataset for this round (bge-m3-lemur-r3).
#   Override to reuse a prior round's data, e.g. bge-m3-lemur-r1.
...
CKPT_REPO="${HF_USER}/bge-m3-lemur-${RN}-checkpoints"
...
plt.title(f"bge-m3-lemur-lora {RN} — Training Loss")
plt.xlabel("Step")
plt.ylabel("Loss")
plt.grid(True)
plt.tight_layout()
...
GGUF_PATH="${GGUF_DIR}/bge-m3-lemur-${RN}-q8_0.gguf"
...
tags:
  ...
  - bge-m3
---
# bge-m3-lemur-${RN} (GGUF q8_0)

Fine-tuned on LEMUR dataset.
Dataset: [${HF_DATASET}](https://huggingface.co/datasets/${HF_DATASET})

## Usage (llama.cpp)
\`\`\`bash
./llama-embedding -m bge-m3-lemur-${RN}-q8_0.gguf --prompt "your query"
\`\`\`
EOF
